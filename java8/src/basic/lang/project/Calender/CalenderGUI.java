...
public class CalenderGUI {
    ...
    private static void printCalender() {
        ...
        inputDate();
        JOptionPane.showMessageDialog(null,printCalender(sumDay()),"Calender",0);
    }
    ...
    */
    private static void inputDate(){
        do {
            String str = JOptionPane.showInputDialog(null, "input a date like 1900-1（1900-5000）:", "Calender", 0);
            String[] tempstr;
            String delimeter = "-";  // 指定分割字符， （. 号需要转义）
            tempstr = str.split(delimeter); // 分割字符串
            for (int i = 0; i < tempstr.length; i++) {
            ...
            JOptionPane.showMessageDialog(null, "Month must between 1-12!!!", "ERROR", JOptionPane.WARNING_MESSAGE);
        } while (true);
    }
    ...
    //3.打印月历
    public static String printCalender(int days) {
