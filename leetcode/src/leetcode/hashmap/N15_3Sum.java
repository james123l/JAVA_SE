...
public class N15_3Sum {
    ...
    public List<List<Integer>> threeSumFast(int[] nums) {
        ...
        if (zeroCount >= 3)  result.add(Arrays.asList(0, 0, 0));
        ...
        int[] negNumMap = new int[negCount];     // Array of all possible negative 3sum values.
        int[] posNumMap = new int[posCount];     // Array of all possible positive 3sum values.
        int posStart = 0;
        ...
        for (int num : nums) {                      // Loop through all numbers in passed array.
            if (num >= minVal && num <= maxVal) {   // Skip numbers that cannot possibly make a 3sum because
                ...
                if (numMap[num - minVal]++ != 0) {  // Count an occurrence if this number.  If already
                    ...
                    if (num > 0) {
                        posNumMap[posCount++] = num; // Add unique positive numbers to positive map.
                    } else if (num < 0) {
                        negNumMap[negCount++] = num; // Add unique negative numbers to negative map.
                    }
                }
            }
        }
        ...
        Arrays.parallelSort(posNumMap, 0, posCount);
        Arrays.parallelSort(negNumMap, 0, negCount);
        ...
        for (int i = negCount - 1; i >= 0; i--) {
            ...
            while (posStart < posCount && posNumMap[posStart] < minpv)  posStart++;
            ...
            for (int j = posStart; j < posCount; j++) {  // Scan through possible pos values for this 3sum.
                ...
                if (ln >= nv && ln <= pv) {         // If the calculated third 3sum value is not between
                    ...
                    if (numMap[ln - minVal] == 0) { // If the calculated third value for the 3sum does not
                        ...
                        // if that number occurs more than once in the original
                        // passed array.
